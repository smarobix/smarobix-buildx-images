...
- **`SDKIMAGE_FEATURES = "dev-pkgs"`.** The SDK is for building against, not for debugging on. Measured on the KV260 SDK built on 2026-09-15: with the default `dev-pkgs dbg-pkgs src-pkgs`, debug symbols made up 72 % of that SDK's target sysroot and debug sources another 8 %. Dropping them shrank the installer from 1.1 GB to 272 MB. See [Tested hardware](../reference/tested-hardware.md#image-sizes) for the published sizes.
